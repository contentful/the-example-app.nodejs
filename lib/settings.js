...
module.exports = async function settingsMiddleware (request, response, next) {
  ...
  const { space_id, environment, preview_token, delivery_token } = request.query // eslint-disable-line camelcase
  if (space_id && environment && preview_token && delivery_token) { // eslint-disable-line camelcase
    settings = {
      ...settings,
      spaceId: space_id,
      environment,
